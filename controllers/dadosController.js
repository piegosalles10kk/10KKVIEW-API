...
const getAllMachineAliases = async (req, res) => {
  try {
    ...
    let files;
    try {
      files = await fs.readdir(targetDirectoryPath);
    } catch (dirError) {
...
const getRelatorioCompleto = async (req, res) => {
  try {
    const historicoPath = 'E:\\DEV\\MONITOR\\relatórios\\historico-de-chamados.json';
    const data = await fs.readFile(historicoPath, 'utf-8');
